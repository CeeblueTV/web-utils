...
describe('ByteRate', () => {
    ...
    describe('basic operations', () => {
        ...
        it('should combine bytes added at the same time', () => {
            const rate = new ByteRate(1000);
            rate.addBytes(500);
            rate.addBytes(500);
            currentTime += 1000;
        ...
        it('should decay to zero when no bytes are added for longer than the interval', () => {
            const rate = new ByteRate(1000);
            rate.addBytes(1000);
            currentTime += 2000;
            expect(rate.value()).toBe(0);
        ...
        it('should keep the observation window bounded before the first sample', () => {
            const rate = new ByteRate(1000);
            currentTime += 5000;
            rate.addBytes(1000);
            expect(rate.value()).toBe(1000);
